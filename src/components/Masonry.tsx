...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const grid = useMemo<GridItem[]>(() => {
    if (!width) return [];
    const activeColumns = Math.min(columns, items.length);
    const colHeights = new Array(activeColumns).fill(0);
    const gap = 16;
    const totalGaps = (activeColumns - 1) * gap;
    const columnWidth = (width - totalGaps) / activeColumns;

    return items.map((child) => {
      const col = colHeights.indexOf(Math.min(...colHeights));
      const x = col * (columnWidth + gap);
      const calculatedHeight = (columnWidth / 600) * child.height;
  ...
  useLayoutEffect(() => {
    ...
    if (container) {
      ...
      gsap.to(container, { height: maxHeight, duration: 0, ease: 'none' });
    }
