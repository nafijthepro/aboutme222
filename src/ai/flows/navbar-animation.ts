...
export async function generateNavbarAnimation(
  ...
  if (!process.env.GEMINI_API_KEY && !process.env.GOOGLE_API_KEY) {
    return {
      animationDescription: '0% { transform: scale(1); background-color: var(--primary); } 50% { transform: scale(1.2); background-color: var(--accent); } 100% { transform: scale(1); background-color: var(--primary); }'
    };
  }
